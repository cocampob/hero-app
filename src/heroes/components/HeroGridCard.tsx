...
export const HeroGridCard = ({ hero }: Props) => {
    ...
    const { toggleFavorites, isFavorite } = use(FavoriteHeroContext);
        ...
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img
                    src={hero.image}
                    alt={hero.name}
                    className="object-cover transition-all duration-500 group-hover:scale-110 group-hover:scale-110 absolute top-[-30px] w-full h-[410px]"
                    onClick={handleClick}
                />
